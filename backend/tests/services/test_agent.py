...
async def main():
    ...
    print(f"\nUser: {query1}")
    response1 = await travel_agent.run_query(query1)
    ...
    query2 = f"Find non-stop flights from SFO to LAX for 2 adults on {departure_date_str} returning {return_date_str}, max price $300."
    print(f"\nUser: {query2}")
    response2 = await travel_agent.run_query(query2)
    print(f"Trava: {response2}")

    # Example 3: Hotel Query (ensure dates are in the future)
    check_in_date_str = (date.today() + timedelta(days=60)).isoformat()
    check_out_date_str = (date.today() + timedelta(days=65)).isoformat()
    query3 = f"Find a 4-star hotel in Rome for 1 adult checking in on {check_in_date_str} and checking out on {check_out_date_str}."
    print(f"\nUser: {query3}")
    response3 = await travel_agent.run_query(query3)
    print(f"Trava: {response3}")

    # Example 4: Currency query
    query4 = "How many Japanese Yen can I get for 1 US Dollar?"
    print(f"\nUser: {query4}")
    response4 = await travel_agent.run_query(query4)
    ...
    while True:
        ...
        if user_input.lower() == 'exit':
            break
        agent_response = await travel_agent.run_query(user_input)
